...
## Application
I took the text similarity and KNN recommendations systems and search engine and deployed them via a Flask application.
Ensure that the filtering criteria doesn't have more than 2000 or so results otherwise you may be waiting for results for
quite a long time due to limited server resources. You can interact with it
<a href="http://3.136.157.114:8080/" target="_blank" rel="noopener noreferrer">here</a> provided it's running
(please email me if it's not). Note that the Matrix Factorization recommender is omitted from this 
